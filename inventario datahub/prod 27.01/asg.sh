...
# Lista de ARNs das Auto Scaling Groups
ASG_ARNS=(
...
# Monta a lista de estruturas --tags
TAGS=()
for ARN in "${ASG_ARNS[@]}"; do
  ASG_NAME="${ARN##*/}"  # pega tudo após 'autoScalingGroupName/'

  TAGS+=("ResourceId=${ASG_NAME},ResourceType=auto-scaling-group,Key=AppID,Value=23008,PropagateAtLaunch=true")
  TAGS+=("ResourceId=${ASG_NAME},ResourceType=auto-scaling-group,Key=CostString,Value=1800.BR.134.602018,PropagateAtLaunch=true")
  TAGS+=("ResourceId=${ASG_NAME},ResourceType=auto-scaling-group,Key=Environment,Value=prd,PropagateAtLaunch=true")
done
...
echo "Tags aplicadas com sucesso nas ASGs (${#ASG_ARNS[@]} grupos)."