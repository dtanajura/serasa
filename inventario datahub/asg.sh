...
# Extrai os nomes e monta o array de estruturas --tags esperado
TAGS_JSON=()
for ARN in "${ASG_ARNS[@]}"; do
  NAME="${ARN##*/}"  # pega o que vem depois de 'autoScalingGroupName/'
  TAGS_JSON+=("ResourceId=$NAME,ResourceType=auto-scaling-group,Key=Environment,Value=stg,PropagateAtLaunch=true")
done
