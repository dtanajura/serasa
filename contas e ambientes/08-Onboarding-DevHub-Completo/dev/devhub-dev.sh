## Roteiro onboarding conta devhub-dev
### 1 - login
.\saml2aws.exe login -a eec-aws-br-eits-devhub-dev

## Verificar elementos previamente instalados na conta
aws ce get-cost-and-usage --profile devhub-dev --no-verify-ssl --granularity "MONTHLY" --time-period Start=2023-10-01,End=2023-10-31 --metrics "UsageQuantity" --group-by Type=DIMENSION,Key=SERVICE --output text
# Listar instâncias
aws ec2 describe-instances --profile devhub-dev --no-verify-ssl --query "Reservations[].Instances[].[InstanceId,Tags[?Key=='Name'].Value]" --output text
# Listar Buckets S3
aws s3 ls --profile devhub-dev --no-verify-ssl
# Listar bancos RDS
aws rds describe-db-instances --profile devhub-dev --no-verify-ssl --query "DBInstances[].[DBInstanceIdentifier]" --output text
# Listar cluster EKS
aws eks list-clusters --profile devhub-dev --no-verify-ssl
# Listar chaves KMS
aws kms list-keys --profile devhub-dev --no-verify-ssl

## Limpar instancias
aws ec2 terminate-instances --instance-ids i-06989fdf2d1415d2b --profile devhub-dev --no-verify-ssl

## Limpar RDS
aws rds delete-db-instance --db-instance-identifier eitsenterprise-dev-devhubportal01 --skip-final-snapshot --profile devhub-dev --no-verify-ssl
aws rds delete-db-instance --db-instance-identifier eitsenterprise-dev-devhubportalqa --skip-final-snapshot --profile devhub-dev --no-verify-ssl

## Passo 01 - onboarding - executar o CloudFormation
aws cloudformation deploy --template-file assumeRole.yaml --stack-name ServiceCatalog --profile devhub-dev --no-verify-ssl
aws iam list-role-policies --role-name BURoleForDevSecOpsCockpitService --profile devhub-dev --no-verify-ssl
aws iam delete-role-policy --role-name BURoleForDevSecOpsCockpitService --policy-name BUPolicyForUseDevSecOpsServiceCatalog --profile devhub-dev --no-verify-ssl
aws iam delete-role --role-name BURoleForDevSecOpsCockpitService --profile devhub-dev --no-verify-ssl
## Passo 02 - executar no Cockpit - aws-onboarding-new-account
# NEW_AWS_ACCOUNT_ID: 977554819825
# DOMAIN: .serasa.intranet
# AWS_REGION: sa-east1
# ENV: snd
# TRIBE: devhub
# VPC_ID: vpc-00c8ba3ca859c6546 
### Listar as VPCs
aws ec2 describe-vpcs --profile devhub-dev --no-verify-ssl --query "Vpcs[].{name:Tags[?Key==`Name`].Value[],ID:VpcId}" --output text
# SUBNET A: subnet-086adfca9bc8df6aa
# SUBNET B: subnet-0424f48254d1772e8
# SUBNET C: subnet-09074fc0cb8b3b0bd
# SUBNET D: 
aws ec2 describe-subnets --profile devhub-dev --no-verify-ssl --query "Subnets[].[Tags[?Key==`Name`].Value,SubnetId]" --filters "Name=tag:Name,Values=aws*" --output text
...
aws ec2 describe-subnets --profile devhub-dev --no-verify-ssl --query "Subnets[].[Tags[?Key==`Name`].Value,CidrBlock]" --filters "Name=tag:Name,Values=aws*" --output text
...
aws ec2 describe-vpcs --profile devhub-dev --no-verify-ssl --query "Vpcs[].[Tags[?Key==`Name`].Value[],CidrBlock]" --output text
